...
function renderShell(){
 ...
 $('#profile-name').textContent=u.full_name; $('#profile-role').textContent=roleName(u.role);
 const nav=roleNavigation(u.role);
 ...
 $('#avatar').onclick=()=>{$('#profile-dropdown').classList.toggle('hidden');$('#avatar').setAttribute('aria-expanded',String(!$('#profile-dropdown').classList.contains('hidden')))};
 $('#profile-dropdown [data-profile-page]').onclick=()=>{loadPage('settings');$('#profile-dropdown').classList.add('hidden')};
 $('#profile-logout').onclick=()=>$('#logout').click();
 $('#notifications').onclick=()=>toast('You have no new notifications.');
}
...
function roleNavigation(role){
 if(role==='student') return [['dashboard','▦ Dashboard'],['clearance','◉ Clearance Status'],['print','▤ Printable Clearance'],['settings','⚙ Settings']];
 if(role==='adviser') return [['dashboard','▦ Dashboard'],['section','♙ Advisory Section'],['requirements','▤ Requirements'],['settings','⚙ Settings']];
 if(role==='sds') return [['dashboard','▦ Dashboard'],['section','♙ Section Clearance'],['requirements','▤ Requirements'],['settings','⚙ Settings']];
 if(role==='dean') return [['dashboard2','▦ Dashboard 2'],['dashboard','▦ Dashboard'],['settings','⚙ Settings']];
 if(role==='program_head') return [['dashboard','▦ Dashboard'],['section','♙ Section Clearance'],['settings','⚙ Settings']];
 if(role==='lab'||role==='library') return [['dashboard','▦ Dashboard'],['section','♙ Section Clearance'],['requirements','⌕ Requirement Settings'],['settings','⚙ Settings']];
 if(role==='cashier') return [['dashboard','▦ Dashboard'],['section','♙ Section Clearance'],['requirements','⌕ Requirement Settings'],['settings','⚙ Settings']];
 return [['dashboard','▦ Dashboard'],['section','♙ Section Clearance'],['requirements','⌕ Requirement Settings'],['settings','⚙ Settings']];
}
function setActive(p){$$('.nav-item').forEach(x=>x.classList.toggle('active',x.dataset.page===p))}
async function loadPage(p){
 setActive(p);
 $('#app-view').classList.toggle('no-topbar', !['dashboard','dashboard2'].includes(p));
 const page=$('#page');
 page.innerHTML='<div class="panel">Loading…</div>';
 try{
  if(p==='dashboard'){await dashboardPage();setupOfficeDashboardFilters()}
  else if(p==='dashboard2') await dashboardPage(true);
...
async function dashboardPage(secondary=false){
 const j=await api('api/dashboard.php?action=summary'), d=j.data, p=$('#page');
 if(d.role==='student'){
  const pct=Math.round(d.cleared/d.total*100);
  p.innerHTML=`<div class="page-head"><div><h1>${esc(APP.user.full_name)}</h1><p>Student • ${esc(APP.user.section||[APP.user.course,APP.user.year_level].filter(Boolean).join(' '))}</p></div><span class="select">AY 2025-2026 ▾</span></div>
  ...
  const c=d.counts||{};
  if(secondary){
   p.innerHTML=`<div class="page-head"><div><h1>${esc(roleName(APP.user.role))} Overview</h1><p>At-a-glance clearance activity for ${esc(APP.user.course||'all assigned courses')}.</p></div><span class="select">AY 2025-2026 ▾</span></div><div class="cards">${stat(c.assigned||0,'Students Assigned')}${stat(c.cleared||0,'Approved')}${stat(c.pending||0,'Pending')}</div><div class="panel"><div class="panel-title">Quick actions</div><div class="quick-actions"><button class="btn primary" onclick="loadPage('section')">Review section clearance</button><button class="btn ghost" onclick="loadPage('settings')">Open account settings</button></div></div>`;
   return;
  }
  const dashboardTitle=APP.user.role==='adviser'?'Advisory Dashboard':APP.user.role==='dean'?'Dean Dashboard':APP.user.role==='program_head'?'Program Dashboard':`${esc(d.department)} Dashboard`;
  p.innerHTML=`<div class="page-head"><div><h1>${dashboardTitle}</h1><p>Signatory: ${esc(APP.user.full_name)}<br>Assigned Course: ${esc(APP.user.course||'BSIT & BSIS')}</p></div><span class="select">AY 2025-2026 ▾</span></div>
  <div class="cards">${stat(c.assigned||0,'Students Assigned')}${stat(c.cleared||0,'Cleared')}${stat(c.pending||0,'Pending')}</div>
  <div class="panel"><div class="panel-title">Sections requiring action</div><div class="table-tools"><label class="search-field"><span>Search sections</span><input id="office-section-search" type="search" placeholder="Search section or course"></label><label class="filter-field"><span>Status</span><select id="office-section-filter"><option value="all">All sections</option><option value="pending">Pending action</option><option value="approved">Approved</option></select></label></div><div class="table-wrap"><table class="data-table"><thead><tr><th>SECTION</th><th>COURSE</th><th>TOTAL</th><th>APPROVED</th><th>PENDING</th><th>ACTION</th></tr></thead><tbody>${d.sections.map(x=>`<tr data-office-section-row data-status="${x.pending?'pending':'approved'}" data-search="${esc(`${x.section} ${x.course}`.toLowerCase())}"><td>${esc(x.section)}</td><td>${esc(x.course)}</td><td>${x.total}</td><td>${x.cleared}</td><td>${x.pending}</td><td><button class="action-btn primary" onclick="openSection('${esc(x.section)}')">Open</button></td></tr>`).join('')}</tbody></table></div></div>
...
function status(s){const label=s==='cleared'?'Approved':s==='rejected'?'Rejected':'Pending';return `<span class="status ${s==='cleared'?'approved':s==='rejected'?'rejected':'pending'}">● ${label}</span>`}
...
async function sectionPage(section){
 ...
 $('#select-all-students').onchange=e=>{$$('.exclude').forEach(input=>{if(!input.closest('tr').hidden)input.checked=e.target.checked})};
 $$('.exclude').forEach(input=>input.onchange=syncSelectAll);
}
function syncSelectAll(){const visible=$$('.exclude').filter(x=>!x.closest('tr').hidden);$('#select-all-students').checked=visible.length>0&&visible.every(x=>x.checked)}
function sectionTable(d){
 const title=APP.user.role==='adviser'?'Advisory Section':'Section Clearance';
 return `<div class="section-screen"><div class="section-head"><div><h1>${title} - ${esc(d.section)}</h1><p>"Clear All" marks every student below as cleared for ${esc(roleName(APP.user.role))} in one action.<br>Tick a box then "Exclude Selected" to leave specific students out.</p></div><div class="row"><button class="btn ghost" onclick="excludeSelected()">Exclude Selected</button><button class="btn primary" onclick="clearAll()">Clear All</button></div></div><div class="section-table-wrap"><table class="data-table section-table"><thead><tr><th><input id="select-all-students" type="checkbox" aria-label="Select all"></th><th>STUDENT NO.</th><th>NAME</th><th>SECTION</th><th>STATUS</th><th>REMARKS</th></tr></thead><tbody>${d.students.map(x=>`<tr data-student-row data-status="${esc(x.status)}" data-search="${esc(`${x.student_no} ${x.full_name}`.toLowerCase())}"><td><input class="exclude" type="checkbox" value="${x.id}" aria-label="Select ${esc(x.full_name)}"></td><td>${esc(x.student_no)}</td><td>${esc(x.full_name)}</td><td>${esc(x.section)}</td><td>${status(x.status)}</td><td>${esc(x.remarks||'—')}</td></tr>`).join('')}</tbody></table></div></div>`;
}
function filterSectionRows(){const query=$('#section-search').value.toLowerCase().trim(),filter=$('#section-filter').value;$$('[data-student-row]').forEach(row=>row.hidden=!!((query&&!row.dataset.search.includes(query))||(filter!=='all'&&row.dataset.status!==filter)))}
...
async function clearAll(mode='all'){
 const visibleInputs=$$('.exclude').filter(x=>!x.closest('tr').hidden);
 const all=visibleInputs.map(x=>Number(x.value));
 const checked=visibleInputs.filter(x=>x.checked).map(x=>Number(x.value));
 const exclude=mode==='exclude' ? checked : [];
 if(!all.length)return toast('No students are available in this section.','error');
 if(mode==='exclude' && !exclude.length)return toast('Select the student(s) you want to exclude.','error');
 const clearCount=all.length-exclude.length;
 if(!await confirmDialog(mode==='exclude'?'Exclude selected students?':'Clear all students?',mode==='exclude'?`Clear ${clearCount} student(s) and leave ${exclude.length} selected student(s) uncleared.`:`Mark all ${all.length} student(s) as cleared.`))return;
 const remarksMap={};
 if(exclude.length){
   $('#modal-root').innerHTML=`<div class="modal-backdrop"><div class="modal"><h2>Remarks for excluded students</h2><p class="muted">Every student left uncleared must have a remark.</p><form id="bulk-remarks">${exclude.map(id=>{
     const row=$$('.exclude').find(x=>Number(x.value)===id)?.closest('tr');
     const name=row?.children[2]?.textContent||'Student';
     return `<label>${esc(name)}<textarea name="remark_${id}" rows="2" required placeholder="Reason for not clearing this student"></textarea></label>`;
   }).join('')}<div class="modal-actions"><button type="button" class="btn ghost" onclick="closeModal()">Cancel</button><button class="btn primary">Continue</button></div></form></div></div>`;
   const rf=$('#bulk-remarks');
   const submitted=await new Promise(resolve=>{
     rf.onsubmit=e=>{e.preventDefault();if(!rf.reportValidity())return;exclude.forEach(id=>remarksMap[String(id)]=String(rf.elements[`remark_${id}`].value).trim());closeModal();resolve(true)};
   });
   if(!submitted)return;
 }
 try{
   await api('api/clearance.php?action=clear_all',{method:'POST',body:JSON.stringify(csrfData({section:currentSection,student_ids:all,exclude,exclude_remarks_map:remarksMap}))});
   toast('Section clearance updated.'); await refreshSection();
 }catch(e){toast(e.message,'error')}
}
async function excludeSelected(){await clearAll('exclude')}
function editClearance(id,status0,remarks0){
 $('#modal-root').innerHTML=`<div class="modal-backdrop"><div class="modal"><h2>Update Clearance</h2><form id="edit-clearance"><input type="hidden" name="student_id" value="${id}"><label>Status<select name="status"><option value="cleared" ${status0==='cleared'?'selected':''}>Approved</option><option value="pending" ${status0==='pending'?'selected':''}>Uncleared</option><option value="rejected" ${status0==='rejected'?'selected':''}>Rejected</option></select></label><label style="margin-top:12px">Remarks<span class="required-hint ${status0==='cleared'?'hidden':''}"> Required when uncleared or rejected</span><textarea name="remarks" rows="3" ${status0!=='cleared'?'required':''}>${esc(remarks0)}</textarea></label><div class="modal-actions"><button type="button" class="btn ghost" onclick="closeModal()">Cancel</button><button class="btn primary">Save</button></div></form></div></div>`;
 const form=$('#edit-clearance'),statusInput=form.elements.status,remarksInput=form.elements.remarks;
 statusInput.onchange=()=>{remarksInput.required=statusInput.value!=='cleared';remarksInput.closest('label').querySelector('.required-hint').classList.toggle('hidden',statusInput.value==='cleared')};
...
async function requirementsPage(){
 ...
 $('#page').innerHTML=`<div class="requirements-screen"><div class="requirements-head"><div><h1>Requirements Settings - ${esc(d.department)}</h1><p>Manage the checklist students must complete for this office.</p></div><button class="btn primary" onclick="requirementModal()">+ Add Requirement</button></div><div class="requirements-panel"><table class="data-table requirements-table"><thead><tr><th>REQUIREMENT</th><th>VISIBLE TO STUDENTS</th><th>TYPE</th><th>ACTION</th></tr></thead><tbody>${d.requirements.filter(x=>x.is_active==1).map(x=>`<tr data-requirement-row data-visibility="${x.visibility?'visible':'hidden'}" data-search="${esc(x.requirement_text.toLowerCase())}"><td>${esc(x.requirement_text)}</td><td><label class="switch"><input type="checkbox" ${x.visibility?'checked':''} onchange="toggleReq(${x.id},this.checked)"><span class="slider"></span></label></td><td>Checkbox</td><td><button class="action-btn" onclick="requirementModal(${x.id},'${esc(x.requirement_text)}',${x.visibility})">Edit</button></td></tr>`).join('')}</tbody></table><div class="requirements-note">Visible requirements are displayed to students on their clearance status page.</div></div></div>`;
}
function filterRequirements(){const query=$('#requirement-search').value.toLowerCase().trim(),filter=$('#requirement-filter').value;$$('[data-requirement-row]').forEach(row=>row.hidden=!!((query&&!row.dataset.search.includes(query))||(filter!=='all'&&row.dataset.visibility!==filter)))}
function setupOfficeDashboardFilters(){if(!$('#office-section-search'))return;$('#office-section-search').oninput=filterOfficeDashboardRows;$('#office-section-filter').onchange=filterOfficeDashboardRows}
function filterOfficeDashboardRows(){const query=$('#office-section-search').value.toLowerCase().trim(),filter=$('#office-section-filter').value;$$('[data-office-section-row]').forEach(row=>row.hidden=!!((query&&!row.dataset.search.includes(query))||(filter!=='all'&&row.dataset.status!==filter)))}
...
function setupPasswordToggles(root=document){root.querySelectorAll('[data-password-toggle]').forEach(button=>{button.textContent='\u{1F441}';button.onclick=()=>{const input=button.parentElement.querySelector('input');const visible=input.type==='text';input.type=visible?'password':'text';button.setAttribute('aria-label',visible?'Show password':'Hide password');button.title=visible?'Show password':'Hide password'}})}
function setupAuth(){
 ...
 $('#forgot-form').onsubmit=async e=>{e.preventDefault();try{const j=await api('api/auth.php?action=forgot',{method:'POST',body:JSON.stringify(csrfData(formData(e.target)))});const result=$('#reset-result');if(j.data.reset_url){result.innerHTML=`${esc(j.message)} <a href="${esc(j.data.reset_url)}">Open reset page</a>`;result.classList.remove('hidden')}else{result.textContent=j.message;result.classList.remove('hidden')}toast(j.message)}catch(x){toast(x.message,'error')}};
 ...
 $('#logout').onclick=async()=>{if(!await confirmDialog('Sign out?','Your current session will be closed.'))return;try{await api('api/auth.php?action=logout',{method:'POST',body:JSON.stringify({csrf:APP.csrf})})}finally{location.reload()}};
}
