...
if (($_GET['action'] ?? '') === 'logout') {
    ...
    if (!empty($_COOKIE[REMEMBER_COOKIE])) {
        $pdo->prepare('DELETE FROM remember_tokens WHERE token_hash=?')->execute([hash('sha256',(string)$_COOKIE[REMEMBER_COOKIE])]);
    }
    setcookie(REMEMBER_COOKIE, '', remember_cookie_options(time()-3600));
    setcookie(session_name(), '', remember_cookie_options(time()-3600));
...
if (($_GET['action'] ?? '') === 'forgot') {
    ...
    if (!$account) json_out(true,'If that email belongs to an active account, reset instructions are ready.', $response);
    if ($account) {
        ...
        $sent=@mail($account['email'],$subject,$message,'Content-Type: text/plain; charset=UTF-8');
        if (!$sent) $response['reset_url']=$resetUrl;
        audit($pdo,(int)$account['id'],'request_password_reset');
    }
    json_out(true,$response['reset_url']?'Email delivery is unavailable locally. Use the generated reset link below.':'If that email belongs to an active account, reset instructions are ready.',$response);
}
...
if (($_GET['action'] ?? '') === 'login') {
    ...
    setcookie(session_name(), session_id(), remember_cookie_options($remember?time()+2592000:0));
    setcookie(REMEMBER_COOKIE, '', remember_cookie_options(time()-3600));
    if ($remember) {
        $rawToken=bin2hex(random_bytes(32));
        $pdo->prepare('DELETE FROM remember_tokens WHERE user_id=? OR expires_at<NOW()')->execute([$u['id']]);
        $pdo->prepare('INSERT INTO remember_tokens(user_id,token_hash,expires_at) VALUES(?,?,DATE_ADD(NOW(), INTERVAL 30 DAY))')->execute([$u['id'],hash('sha256',$rawToken)]);
        setcookie(REMEMBER_COOKIE,$rawToken,remember_cookie_options(time()+2592000));
    }
    $_SESSION['user']=session_user_from_row($u);
    csrf_token(); audit($pdo,(int)$u['id'],'login');
    json_out(true,'Welcome back.', ['user'=>$_SESSION['user'],'csrf'=>$_SESSION['csrf']]);
}

if (($_GET['action'] ?? '') === 'register') {
    $required=['first_name','last_name','student_no','course','year_level','semester','section','email','contact_no','password','confirm_password'];
